...
function xxx(options, dayjsClass, dayjsFactory) {
    const classProto = dayjsClass.prototype;

    const oldFormat = classProto.format;
    const oldParse = classProto.parse;
    let isJalali = false;
    classProto.doAsJalali = function (name) {
        if (name === 'fa') {
            dayjsFactory.locale(jalaliFaLocale, null, true);
        } else {
            dayjsFactory.locale(jalaliEnLocale, null, true);
        }

        isJalali = true;
        return this;
    }
    classProto.doAsGregorian = function (name) {
        if (name === 'fa') {
            dayjsFactory.locale(gregorianFaLocale, null, true);
        }
        isJalali = false;
        return this;
    }
    ...
    classProto.format = function (formatStr) {
        ...
        const fff = gregorianFaLocale;
        if (isJalali || locale.name === 'fa') {
